...
public class DKSwing {
    ...
    public static void main(String[] args){
        ...
        int i = aSwing.searchContinuityAboveValue(SwingSample.column.WZ, 1,1200, 1, 20);
        System.out.println("From index [1 - 1200] for sensor 'wz', Above Value 1 of length >= " + 20 + "\n\t: Index["+i+"]");

        int x = aSwing.backSearchContinuityWithinRange(SwingSample.column.AX, 860,1, 1,
                16, 20 );
        System.out.println("From index [860 - 1] for sensor 'ax', Above Value 1 and below value 16 of length >= 20 \n\t: Index["+x+"]");

        int y = aSwing.searchContinuityAboveValueTwoSignals(SwingSample.column.AX, SwingSample.column.WX, 1, 1200, 0,
                0, 5);
        System.out.println("From index [1 - 1200] for sensor 'ax' above value 0 and for sensor wx above value 0, of length >= 5 \n\t: Index["+y+"]");

        ArrayList<IndexPair> testList = aSwing.searchMultiContinuityWithinRange(SwingSample.column.AY, 1, 1200,
                2, 3, 3);
